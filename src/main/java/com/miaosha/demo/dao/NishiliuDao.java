...
public interface NishiliuDao {
    ...
    void Insert(@Param("cr") Nishiliu cr);

    @Select("select * from nishiliu ORDER BY `key` ASC")
    List<Nishiliu> selectAll();

    @Select("select * from nishiliu where type = #{type}")
    List<Nishiliu> selectByType(@Param("type") String type);

    @Select("select * from nishiliu where reporting_unit = #{reporting_unit}")
    List<Nishiliu> selectByUnit(@Param("reporting_unit") String reporting_unit);
    
    @Select("select * from nishiliu where `key` = #{key}")
    List<Nishiliu> selectByKey(@Param("key") String key);
    ...
    boolean insertForeach(@Param(value = "list") List<Nishiliu> list);
    ...
    boolean beifen(@Param(value = "list") List<Nishiliu> list);
    ...
    void updateByKey(@Param("cr") Nishiliu cr);
}
