...
public interface ZhenqingJibenDao {
    ...
    void Insert(@Param("item") ZhenqingJiben item);

    @Select("select * from zhenqingjiben ORDER BY `key` ASC")
    List<ZhenqingJiben> selectAll();

    @Select("select * from zhenqingjiben where type = #{type}")
    List<ZhenqingJiben> selectByType(@Param("type") String type);

    @Select("select * from zhenqingjiben where reporting_unit = #{reporting_unit}")
    List<ZhenqingJiben> selectByUnit(@Param("reporting_unit") String reporting_unit);
    
    @Select("select * from zhenqingjiben where `key` = #{key}")
    List<ZhenqingJiben> selectByKey(@Param("key") String key);
    ...
    boolean insertForeach(@Param(value = "list") List<ZhenqingJiben> list);
    ...
    boolean beifen(@Param(value = "list") List<ZhenqingJiben> list);
    ...
    void updateByKey(@Param("item") ZhenqingJiben item);
}
